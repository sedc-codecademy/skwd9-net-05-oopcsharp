...
namespace Exercise03
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            //jerry.FavoriteSongs.AddRange(songs.Where(x => x.Title[0] == 'B'));
            //jerry.FavoriteSongs.AddRange(songs.Where(x => x.Title.Length > 0 && x.Title[0] == 'B'));
            jerry.FavoriteSongs.AddRange(songs.Where(x => x.Title.Trim().StartsWith("B", StringComparison.InvariantCultureIgnoreCase)));
            maria.FavoriteSongs.AddRange(songs.Where(song => song.Length >= 360));
            jane.FavoriteSongs.AddRange(songs.Where(x => x.Genre == GenreEnum.Rock));
            stefan.FavoriteSongs.AddRange(songs.Where(x => x.Genre == GenreEnum.HipHop && x.Length < 180));

            List<Person> peopleWith4SongsOrMore = fansArray.Where(fan => fan.FavoriteSongs.Count >= 4).ToList();
